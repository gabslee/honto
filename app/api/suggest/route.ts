...
const fallbackLies = (_truth: string) => ["I once missed a flight because I followed the wrong stranger.", "I have a hidden talent for naming every song in the first three seconds."];
...
export async function POST(request: Request) {
  ...
  try {
    const instruction = body.kind === "lies"
      ? `You are helping me play Two Lies and One Truth. Truth from the player: "${(body.truth ?? "").slice(0, 180)}". Write exactly two short, believable lies that could fool a close friend. Do not copy, paraphrase, negate, or closely mirror the truth. Do not reuse its people, places, dates, actions, objects, or outcome. Make every lie a completely different everyday event, with natural casual wording. No theme, no explanations, no quotation marks, no hyphens. Return a JSON object with a lies array containing exactly two items.`
      : body.kind === "question"
    ...
    const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: process.env.OPENAI_MODEL ?? "gpt-5.4-nano", store: false, input: instruction, text: { format: { type: "json_schema", name: body.kind === "lies" ? "lie_options" : body.kind === "question" ? "question_options" : "theme_options", strict: true, schema: body.kind === "lies" ? { type: "object", properties: { lies: { type: "array", items: { type: "string" }, minItems: 2, maxItems: 2 } }, required: ["lies"], additionalProperties: false } : body.kind === "question" ? { type: "object", properties: { questions: { type: "array", items: { type: "string" } } }, required: ["questions"], additionalProperties: false } : { type: "object", properties: { themes: { type: "array", items: { type: "string" }, minItems: count, maxItems: count } }, required: ["themes"], additionalProperties: false } } }, max_output_tokens: body.kind === "lies" ? 180 : body.kind === "question" ? 320 : Math.max(180, count * 35) }) });
    ...
    if (body.kind === "lies") {
      ...
      const lies = Array.isArray(parsed.lies) ? parsed.lies.filter((item: unknown): item is string => typeof item === "string").map((item: string) => item.trim().slice(0, 180)).filter(Boolean).slice(0, 2) : [];
      if (lies.length === 2) return Response.json({ lies, source: "ai" });
