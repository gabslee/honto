...
type Body = { kind?: "theme" | "lies"; truth?: string; prompt?: string; category?: keyof typeof themeCategories; exclude?: string[] };

const fallbackTheme = (category: keyof typeof themeCategories = "mixed", exclude: string[] = []) => {
  const list = themeCategories[category] ?? themeCategories.mixed;
...
export async function POST(request: Request) {
  ...
  if (!apiKey || apiKey.startsWith("REPLACE_")) {
    return Response.json(body.kind === "lies" ? { lies: fallbackLies(body.truth ?? "my secret"), source: "fallback" } : { prompt: fallbackTheme(category, body.exclude), source: "fallback" });
  }
  try {
    const instruction = body.kind === "lies"
      ...
      : `Give one fresh, playful English theme for a two-lies-one-truth party game in the ${category} category. Avoid: ${(body.exclude ?? []).join(", ") || "none"}. Return only a 2–8 word noun phrase.`;
