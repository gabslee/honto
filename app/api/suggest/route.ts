...
type FallbackReason = "not_configured" | "quota_exhausted" | "request_failed" | "invalid_response";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model: process.env.OPENAI_MODEL ?? "gpt-5-nano", store: false, input: instruction, text: { format: { type: "json_schema", name: body.kind === "lies" ? "lie_options" : body.kind === "question" ? "question_options" : "theme_options", strict: true, schema: body.kind === "lies" ? { type: "object", properties: { lies: { type: "array", items: { type: "string" }, minItems: 5, maxItems: 5 } }, required: ["lies"], additionalProperties: false } : body.kind === "question" ? { type: "object", properties: { questions: { type: "array", items: { type: "string" }, minItems: count, maxItems: count } }, required: ["questions"], additionalProperties: false } : { type: "object", properties: { themes: { type: "array", items: { type: "string" }, minItems: count, maxItems: count } }, required: ["themes"], additionalProperties: false } } }, max_output_tokens: body.kind === "lies" ? 260 : body.kind === "question" ? 320 : Math.max(180, count * 35) }) });
    ...
    if (body.kind === "lies") {
      ...
      fallbackReason = "invalid_response"; throw new Error("Invalid lie options");
    }
    if (body.kind === "question") {
      ...
      fallbackReason = "invalid_response"; throw new Error("Invalid question options");
    }
    const parsed = JSON.parse(text || "{}");
