...
export async function POST(request: Request) {
  ...
  try {
    const instruction = body.kind === "lies"
      ...
      : `Create ${count} short English topic phrases for a two-lies-one-truth game. The topic should invite one real personal story from the player. Selected subject areas: ${(Array.isArray(categories) ? categories.join(", ") : categories)}. Custom subject: ${(body.customTheme ?? "").slice(0, 80) || "none"}. ${body.customTheme ? "Make every option specifically about the custom subject." : ""} ${body.fresh ? "Make these feel fresh, specific, and unlike generic party game prompts." : ""} Good style: a school situation, your first job, a family tradition, a weird snack, or a time you got lost. Do not write a title, question, instruction, or generic label. Avoid formats like Cozy Confessions: Family Life Edition, Family Life, or Tell your story. Use plain conversational lowercase wording, 2 to 8 words each. No punctuation, quotes, colon, semicolon, question mark, exclamation mark, em dash, en dash, or hyphen. Do not use words such as edition, confession, category, theme, or party game. Avoid: ${(body.exclude ?? []).join(", ") || "none"}. Return a JSON object with a themes array containing exactly ${count} options.`;
