...
export async function POST(request: Request) {
  ...
  try {
    const instruction = body.kind === "lies"
      ? `You are helping me play Two Lies and One Truth. My truth is: "${(body.truth ?? "").slice(0, 180)}". Theme: ${(body.prompt ?? "anything goes").slice(0, 80)}. Write five possible lies in natural English. Keep each lie roughly the same character length as the truth and copy the user's writing style, including casual wording, grammar mistakes, spelling, missing accents, capitalization, and punctuation. Keep only the broad subject or feeling so the lies are believable, but invent a genuinely different story each time. Change the event, people, place, time, or outcome. Never repeat a sequence of four or more words from the truth, never start with the same clause, and never keep the truth's main event while adding extra details. For example, if the truth says someone went out with a guy from a language app, do not write that same date five times with different details. Instead branch into different believable events involving other people or situations. Make every lie distinct and not obvious to a friend. Do not use quotation marks, em dashes, en dashes, or hyphens. Return exactly five statements in a JSON object with a lies array.`
      : body.kind === "question"
