...
export async function POST(request: Request) {
  ...
  try {
    const instruction = body.kind === "lies"
      ? `You are helping me play Two Lies and One Truth. Truth from the player: "${(body.truth ?? "").slice(0, 180)}". Write exactly five short, believable lies that could fool a close friend. Do not copy, paraphrase, negate, or closely mirror the truth. Do not reuse its people, places, dates, actions, objects, or outcome. Make every lie a completely different everyday event, with natural casual wording. No theme, no explanations, no quotation marks, no hyphens. Return a JSON object with a lies array containing exactly five items.`
      : body.kind === "question"
