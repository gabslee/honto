...
import { safeGoogleReturn } from "../../../../google-return.js";
...
export async function GET(request: Request) {
  ...
  const requestedReturn = requestUrl.searchParams.get("return_to") || request.headers.get("referer") || "/";
  const returnTo = safeGoogleReturn(requestedReturn, origin);
  ...
  headers.append("Set-Cookie", `honto_google_state=${state}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=600`);
  headers.append("Set-Cookie", `honto_google_return=${encodeURIComponent(returnTo)}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=600`);
