...
function EstimateCard({ card, meId, busy, act }: { card: Card; meId: string; busy: boolean; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  if (card.status === "ready") return card.actorId === meId ? <div className="play-card estimate-card"><CardBadge type="estimate"/><h2>Give the real number.</h2><div className="estimate-question-wrap"><blockquote className="big-question">{card.payload.question}</blockquote><button type="button" className="estimate-next-button" disabled={busy} onClick={() => void act("nextEstimate")} aria-label="Choose another estimate question">NEXT ESTIMATE ↻</button></div><p className="hint">Your answer stays secret. We will mix it with four believable options.</p><input className="number-answer" type="number" min="0" max="1000000" step="1" inputMode="numeric" value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Your exact answer"/><button className="primary-button" disabled={busy || answer === ""} onClick={() => act("submitEstimate", { correctNumber: Number(answer) })}>LOCK MY ANSWER →</button></div> : <Waiting title={`${card.actorName} is locking in the real number…`} text={card.payload.question ?? "A numeric question is coming."}/>;
  if (card.status === "guess") return card.targetId === meId ? <><div className="play-card estimate-card"><CardBadge type="estimate"/><h2>How well do you know <em>{card.actorName}</em>?</h2><blockquote className="big-question">{card.payload.question}</blockquote><p className="hint"><SipMug/> A wrong guess costs one sip. Keep guessing until you find it.</p><div className="estimate-options">{options.map((option) => <button key={option} className={wrong.includes(option) ? "eliminated" : ""} disabled={busy || wrong.includes(option) || wrongPopup} onClick={() => void act("guessEstimate", { estimate: option })}><strong>{option.toLocaleString()}</strong>{wrong.includes(option) && <small><SipMug/> WRONG · 1 SIP</small>}</button>)}</div><p className="attempt-count">{wrong.length ? <><SipMug/> {wrong.length} wrong {wrong.length === 1 ? "guess" : "guesses"} · {wrong.length} {wrong.length === 1 ? "sip" : "sips"}</> : <><SipMug/> First try: if you nail it, {card.actorName} spins the 1–3 SIP wheel.</>}</p></div>{wrongPopup && <div className="estimate-miss-backdrop"><div className="estimate-miss-card" role="alertdialog" aria-modal="true" aria-labelledby="estimate-miss-title"><div className="estimate-miss-mark">×</div><span className="eyebrow">WRONG GUESS</span><h2 id="estimate-miss-title">Not this time.</h2><p><SipMug/> Take <strong>1 SIP</strong>, then try again.</p><button type="button" className="primary-button" onClick={() => setWrongPopup(false)}>TRY AGAIN →</button></div></div>}</> : <Waiting title={`${card.targetName} is estimating…`} text={`${wrong.length} wrong ${wrong.length === 1 ? "guess" : "guesses"} so far.`}/>;
  return null;
}
...
function Reveal({ card, players, meId, close, spinWheel }: { card: Card; players: Player[]; meId: string; close: () => void; spinWheel: () => Promise<unknown> }) {
  ...
  if (revealPhase === "ready") return <div className="modal-backdrop"><div className="reveal-card wheel-ready" role="dialog" aria-live="polite"><div className="result-mark">{icon}</div><span className="eyebrow"><SipMug/> SIP WHEEL</span><h2>Ready to spin?</h2><p className="reveal-intro-copy">{doubleSips ? "Skipping doubles the wheel: 2, 4, or 6 sips." : card.type === "both" ? "One spin sets the SIPs for both players." : `${spinnerName} takes the wheel.`}</p><div className="sip-wheel-stage wheel-ready-stage"><i className="sip-wheel-pointer"/><div className={`${wheelClass} sip-wheel-static`}><span className="sip-wheel-number one">{doubleSips ? 2 : 1}</span><span className="sip-wheel-number two">{doubleSips ? 4 : 2}</span><span className="sip-wheel-number three">{doubleSips ? 6 : 3}</span></div></div>{meId === spinnerId ? <button type="button" className="wheel-start-button" aria-label="Spin the sip wheel" disabled={requestingSpin} onClick={async () => { setRequestingSpin(true); try { await spinWheel(); } finally { setRequestingSpin(false); } }}>{requestingSpin ? "STARTING THE WHEEL…" : <><SipMug/> SPIN</>}</button> : <p className="wheel-waiting"><strong>{spinnerName}</strong> is ready to spin the wheel…</p>}</div></div>;
  if (revealPhase === "spinning") return <div className="modal-backdrop"><div className="reveal-card wheel-card" role="status" aria-live="polite"><span className="eyebrow"><SipMug/> SIP WHEEL</span><h2>How many sips?</h2><div className="sip-wheel-stage"><i className="sip-wheel-pointer"/><div className={wheelClass}><span className="sip-wheel-number one">{doubleSips ? 2 : 1}</span><span className="sip-wheel-number two">{doubleSips ? 4 : 2}</span><span className="sip-wheel-number three">{doubleSips ? 6 : 3}</span></div></div><p><SipMug/> {spinnerName} is spinning the wheel…</p></div></div>;
