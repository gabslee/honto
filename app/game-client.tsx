...
const CARD_META_JA: Record<keyof typeof CARD_META, { label: string }> = {
  honto: { label: "嘘2つ・本当1つ" }, question: { label: "質問かシップ" }, preference: { label: "心を読む" }, estimate: { label: "数字当て" }, rps: { label: "じゃんけん" }, both: { label: "2人で飲む" },
};
...
function LanguageMenu({ onChange }: { onChange?: (locale: Locale) => void } = {}) { const { locale, setLocale } = useLocale(); return <label className="language-menu"><span aria-hidden="true">文</span><select value={locale} onChange={(event) => { const next = event.target.value as Locale; setLocale(next); onChange?.(next); }} aria-label={locale === "ja" ? "言語を選択" : "Choose language"}><option value="en">EN</option><option value="ja">日本語</option></select></label>; }
...
export default function GameClient() {
  ...
  if (!session) return <LocaleContext.Provider value={{ locale, setLocale }}><Landing name={name} setName={setName} joinCode={joinCode} setJoinCode={setJoinCode} mode={mode} setMode={setMode} enter={enter} busy={busy} error={error} /></LocaleContext.Provider>;
...
function WelcomeScreen({ game, busy, acknowledge }: { game: GameState; busy: boolean; acknowledge: () => Promise<unknown> }) {
  ...
  return <section className="game-stage welcome-stage"><div className="welcome-card"><span className="eyebrow">{ja ? "HONTOへようこそ" : "WELCOME TO HONTO"}</span><div className="welcome-mark" aria-hidden="true"><span>本当</span><b>?!</b></div><h1>{ja ? "Hontoへようこそ。" : "Welcome to Honto."}</h1><p>{ja ? "Hontoは2人で遊ぶ共有カードゲームです。カードを引いてミニゲームを見つけ、お互いを読みながら遊びます。" : "Honto is a shared card game for two. Draw a card, discover a mini game, and read each other as you play."}</p><p className="welcome-flow"><strong>{ja ? "遊び方" : "How it works"}</strong><br/>{ja ? "1人が次のカードを引きます。2人に同じチャレンジが表示され、一緒に遊んで次のカードへ進みます。" : "One player draws the next card. Both players see the challenge, then play it together until the next card."}</p>{acknowledged ? <div className="welcome-waiting"><span className="typing"><i/><i/><i/></span><strong>{ja ? "もう1人の確認を待っています…" : "Waiting for the other player…"}</strong><small>{ja ? "2人が「わかりました」を押すと最初のカードが開きます。" : "They need to tap GOT IT before the first card opens."}</small></div> : <button type="button" className="primary-button welcome-button" disabled={busy} onClick={() => void acknowledge()}>{busy ? (ja ? "保存中…" : "SAVING…") : (ja ? "わかりました →" : "GOT IT →")}</button>}</div></section>;
}

function SkipButton({ busy, skip }: { busy: boolean; skip: () => Promise<unknown> }) {
  const { locale } = useLocale(); const ja = locale === "ja";
  return <button type="button" className="skip-button" title={ja ? "このミニゲームをスキップ（2・4・6シップのルーレット）" : "Skip this mini game · spin for 2, 4 or 6 sips"} aria-label={ja ? "このミニゲームをスキップ" : "Skip this mini game. Spin for 2, 4 or 6 sips."} disabled={busy} onClick={() => void skip()}><span>↷</span><strong>{ja ? "スキップ" : "SKIP"}</strong><small><SipMug/> {ja ? "シップ2倍ルーレット" : "2× SIP WHEEL"}</small></button>;
}
...
function HontoComposer({ card, game, busy, act }: { card: Card; game: GameState; busy: boolean; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  const generate = async () => { setGenerating(true); try { const response = await fetch("/api/suggest", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "lies", truth, locale }) }); const data = await response.json(); if (!response.ok || !Array.isArray(data.lies)) throw new Error("No ideas available."); setLies(data.lies); setSelected([]); } finally { setGenerating(false); } };
...
function Reveal({ card, players, meId, close, spinWheel }: { card: Card; players: Player[]; meId: string; close: () => void; spinWheel: () => Promise<unknown> }) {
  const { locale } = useLocale(); const ja = locale === "ja";
  const hasWheel = Boolean(card.result.skipped) || card.type === "honto" || card.type === "preference" || card.type === "rps" || card.type === "both" || (card.type === "estimate" && card.result.firstTry);
  const doubleSips = Boolean(card.result.skipped);
  const wheelValues = doubleSips ? [2, 4, 6] : [1, 2, 3];
  ...
  if (doubleSips) { const skipper = players.find((player) => player.id === (card.result.skipById ?? card.result.drinkerId))?.name ?? (ja ? "プレイヤー" : "The player"); icon = "↷"; eyebrow = ja ? "ミニゲームをスキップ" : "MINI GAME SKIPPED"; title = ja ? `${skipper}がスキップし、${card.result.sips}シップ。` : `${skipper} skips and takes ${card.result.sips} sips.`; detail = ja ? "スキップ時はルーレットの数字が2倍（2・4・6シップ）です。" : "Skipping doubles the wheel result: 2, 4, or 6 sips."; }
  ...
  const introTitle = doubleSips ? (ja ? "スキップしてシップ！" : "Skip & sip!") : card.type === "rps" ? "Joken-pô!" : card.type === "both" ? (ja ? "みんなで乾杯！" : "Everyone, cheers!") : card.type === "honto" ? (ja ? "本当？ブラフ？" : "Truth or bluff?") : (ja ? "心読み完了！" : "Mind read complete!");
  ...
  if (revealPhase === "ready") return <div className="modal-backdrop"><div className="reveal-card wheel-ready" role="dialog" aria-live="polite"><div className="result-mark">{icon}</div><span className="eyebrow"><SipMug/> {ja ? "シップルーレット" : "SIP WHEEL"}</span><h2>{ja ? "回す準備はできましたか？" : "Ready to spin?"}</h2><p className="reveal-intro-copy">{doubleSips ? (ja ? "スキップでは数字が2倍（2・4・6シップ）です。" : "Skipping doubles the wheel: 2, 4, or 6 sips.") : card.type === "both" ? (ja ? "1回の結果を2人に適用します。" : "One spin sets the SIPs for both players.") : (ja ? `${spinnerName}がルーレットを回します。` : `${spinnerName} takes the wheel.`)}</p><div className="sip-wheel-stage wheel-ready-stage"><i className="sip-wheel-pointer"/><div className={`${wheelClass} sip-wheel-static`}><span className="sip-wheel-number one">{doubleSips ? 2 : 1}</span><span className="sip-wheel-number two">{doubleSips ? 4 : 2}</span><span className="sip-wheel-number three">{doubleSips ? 6 : 3}</span></div></div>{meId === spinnerId ? <button type="button" className="wheel-start-button" aria-label={ja ? "シップルーレットを回す" : "Spin the sip wheel"} disabled={requestingSpin} onClick={async () => { setRequestingSpin(true); try { await spinWheel(); } finally { setRequestingSpin(false); } }}>{requestingSpin ? (ja ? "開始中…" : "STARTING THE WHEEL…") : <><SipMug/> {ja ? "回す" : "SPIN"}</>}</button> : <p className="wheel-waiting"><strong>{spinnerName}</strong> {ja ? "がルーレットを回す準備中…" : "is ready to spin the wheel…"}</p>}</div></div>;
  if (revealPhase === "spinning") return <div className="modal-backdrop"><div className="reveal-card wheel-card" role="status" aria-live="polite"><span className="eyebrow"><SipMug/> {ja ? "シップルーレット" : "SIP WHEEL"}</span><h2>{ja ? "何シップ？" : "How many sips?"}</h2><div className="sip-wheel-stage"><i className="sip-wheel-pointer"/><div className={wheelClass}><span className="sip-wheel-number one">{doubleSips ? 2 : 1}</span><span className="sip-wheel-number two">{doubleSips ? 4 : 2}</span><span className="sip-wheel-number three">{doubleSips ? 6 : 3}</span></div></div><p><SipMug/> {ja ? `${spinnerName}がルーレットを回しています…` : `${spinnerName} is spinning the wheel…`}</p></div></div>;
  return <div className="modal-backdrop"><div className={`reveal-card ${viewerDrinks || card.type === "both" ? "wrong" : "correct"}`}><div className="result-mark">{icon}</div><span className="eyebrow">{eyebrow}</span>{hasWheel && <><div className="sip-wheel-stage wheel-result-stage"><i className="sip-wheel-pointer"/><div className={`${wheelClass} sip-wheel-static sip-wheel-final`} aria-label={ja ? `ルーレットの結果：${card.result.sips}シップ` : `Final wheel result: ${card.result.sips} sips`}><span className="sip-wheel-number one">{doubleSips ? 2 : 1}</span><span className="sip-wheel-number two">{doubleSips ? 4 : 2}</span><span className="sip-wheel-number three">{doubleSips ? 6 : 3}</span></div></div><div className="wheel-result"><span>{ja ? "ルーレットの結果" : "THE WHEEL SAYS"}</span><strong>{card.result.sips}</strong><small><SipMug/> {ja ? "シップ" : card.result.sips === 1 ? "SIP" : "SIPS"}</small></div></>}<h2>{(card.result.drinkerId || card.type === "both") && <SipMug/>} {title}</h2><blockquote>{detail}</blockquote><button className="primary-button" onClick={close}>{ja ? "次のカード →" : "NEXT CARD →"}</button></div></div>;
}

function Finished({ players, leave }: { players: Player[]; leave: () => void }) {
  const { locale } = useLocale(); const ja = locale === "ja";
  const sorted = useMemo(() => [...players].sort((a, b) => a.sips - b.sips), [players]);
  return <section className="finished"><span className="eyebrow">{ja ? "デッキが空になりました" : "THE DECK IS EMPTY"}</span><h1>{ja ? "一番飲まなかったのは…" : "The lightest drinker was…"}</h1><div className="winner">🏆<strong>{sorted[0]?.name}</strong><span><SipMug/> {sorted[0]?.sips} {ja ? "シップ" : sorted[0]?.sips === 1 ? "sip" : "sips"}</span></div><div className="final-list">{sorted.map((player, index) => <div key={player.id}><b>#{index + 1}</b><span>{player.name}</span><small><SipMug/> {player.sips} {ja ? "シップ" : player.sips === 1 ? "sip" : "sips"}</small></div>)}</div><button className="primary-button" onClick={leave}>{ja ? "新しいテーブル →" : "NEW TABLE →"}</button></section>;
}
