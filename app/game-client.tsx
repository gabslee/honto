...
function HontoComposer({ card, game, busy, act }: { card: Card; game: GameState; busy: boolean; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  const generate = async () => { setGenerating(true); setAiNotice(""); try { const response = await fetch("/api/suggest", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "lies", truth, locale }) }); const data = await response.json(); if (!response.ok || !Array.isArray(data.lies)) throw new Error("No ideas available."); setLies(data.lies); setSelected([]); if (data.source !== "ai") setAiNotice(aiFallbackNotice(locale, data.reason)); } catch { setAiNotice(aiFallbackNotice(locale)); } finally { setGenerating(false); } };
...
function QuestionCard({ card, meId, game, busy, act }: { card: Card; meId: string; game: GameState; busy: boolean; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  const [question, setQuestion] = useState(""); const [sips, setSips] = useState(1); const [ideas, setIdeas] = useState<string[]>([]); const [ideaSource, setIdeaSource] = useState<"ai" | "curated" | null>(null); const [aiNotice, setAiNotice] = useState(""); const questionPointer = useRef({ x: 0, y: 0, moved: false });
...
function Reveal({ card, players, meId, close, spinWheel }: { card: Card; players: Player[]; meId: string; close: () => void; spinWheel: () => Promise<unknown> }) {
  ...
  const introTitle = card.result.skipped ? (ja ? "スキップしてシップ！" : "Skip & sip!") : estimateFirstTry ? (ja ? "一発正解！" : "First try!") : card.type === "rps" ? "Joken-pô!" : card.type === "both" ? (ja ? "みんなで乾杯！" : "Everyone, cheers!") : card.type === "honto" ? (ja ? "本当？ブラフ？" : "Truth or bluff?") : (ja ? "心読み完了！" : "Mind read complete!");
