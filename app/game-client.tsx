  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <button className="tiny-button" onClick={leave}>{t.common.exit}</button>
      </header>
  ...
  return <section className="lobby">
    ...
    <div className="lobby-grid">
      ...
      <div className="panel settings-panel"><div className="panel-title"><h2>{t.lobby.rules}</h2><span className="sticker">{t.lobby.yourCall}</span></div>
        <Setting label={t.lobby.length} value={game.room.roundCount} options={[10,20,30]} suffix={t.lobby.roundsSuffix} disabled={!host} onChange={(roundCount) => act("configure", { roundCount, groupSipEvery: game.room.groupSipEvery, timerMinutes: game.room.timerMinutes })}/>
        <Setting label={t.lobby.everyoneSips} value={game.room.groupSipEvery ?? 0} options={[0,3,5]} labels={[t.lobby.never,t.lobby.every3,t.lobby.every5]} disabled={!host} onChange={(groupSipEvery) => act("configure", { roundCount: game.room.roundCount, groupSipEvery: groupSipEvery || null, timerMinutes: game.room.timerMinutes })}/>
        <Setting label={t.lobby.timer} value={game.room.timerMinutes ?? 0} options={[0,10,15]} labels={[t.lobby.off,"10 min","15 min"]} disabled={!host} onChange={(timerMinutes) => act("configure", { roundCount: game.room.roundCount, groupSipEvery: game.room.groupSipEvery, timerMinutes: timerMinutes || null })}/>
