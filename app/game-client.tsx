...
const RPS_OPTIONS: Array<{ value: RpsChoice; icon: string; label: string }> = [
  { value: "rock", icon: "✊", label: "PEDRA" },
  { value: "paper", icon: "✋", label: "PAPEL" },
  { value: "scissors", icon: "✌️", label: "TESOURA" },
];
...
function RpsCard({ card, meId, busy, act }: { card: Card; meId: string; busy: boolean; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  return <><div className="play-card rps-card"><CardBadge type="rps"/><h2>{card.payload.tieCount ? "Empate! Joken-pô de novo." : `Joken-pô contra ${opponent}.`}</h2><p className="hint"><SipMug/> Pick secretly. The loser spins the 1–3 SIP wheel.</p><div className="rps-options">{RPS_OPTIONS.map((option) => <button key={option.value} disabled={busy || showTie} onClick={() => act("chooseRps", { rpsChoice: option.value })}><span>{option.icon}</span><strong>{option.label}</strong></button>)}</div></div>{showTie && <div className="rps-tie-backdrop"><div className="rps-tie-card" role="status" aria-live="polite"><span className="eyebrow">JOKEN-PÔ</span><div className="rps-tie-hands"><span>✊</span><b>VS</b><span>✊</span></div><h2>Empate!</h2><p>Os dois escolheram igual. Preparem-se para tentar de novo.</p><div className="reveal-dots"><i/><i/><i/></div></div></div>}</>;
}
