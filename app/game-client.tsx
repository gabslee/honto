...
export default function GameClient() {
  ...
  return <LocaleContext.Provider value={{ locale, setLocale, roomCode: session.code, sessionToken: session.token }}><main className="app-shell">
    ...
    {game.group && (game.room.status === "playing" || game.room.status === "finished" || game.room.status === "abandoned") && <MultiplayerGame group={game.group} players={game.players} meId={game.meId} busy={busy} act={act} locale={locale} welcomeAck={game.room.welcomeAck} canRestart={Boolean(host && game.room.canHostMultiplayer)} />}
