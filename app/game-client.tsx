...
type GameState = {
  room: { code: string; status: "lobby" | "playing" | "finished" | "abandoned"; roundCount: number; currentRound: number; themeCategory: string; customTheme: string | null; cardTypes?: string; welcomeAck: string[]; locale: Locale; startedAt: string | null; multiplayer?: boolean; maxPlayers?: number; canHostMultiplayer?: boolean; canUseSpicy?: boolean; canCustomizeDeck?: boolean };
  players: Player[]; activeCard: Card | null; lastCard: Card | null; meId: string;
  group?: ReturnType<typeof publicMultiplayer> | null;
};
...
function AccountControl() {
  ...
  useEffect(() => {
    const identity = mount?.querySelector<HTMLElement>(".account-identity");
    if (!identity || !user) return;
    identity.tabIndex = 0; identity.setAttribute("role", "link"); identity.setAttribute("aria-label", "Open my Honto account");
    const openAccount = (event: Event) => { if (!(event.target as Element).closest("button")) window.location.href = "/account"; };
    const onKey = (event: Event) => { if ((event as KeyboardEvent).key === "Enter") window.location.href = "/account"; };
    identity.addEventListener("click", openAccount); identity.addEventListener("keydown", onKey);
    return () => { identity.removeEventListener("click", openAccount); identity.removeEventListener("keydown", onKey); };
  }, [mount, user]);
...
export default function GameClient() {
  ...
  return <LocaleContext.Provider value={{ locale, setLocale, roomCode: session.code, sessionToken: session.token }}><main className="app-shell">
    ...
    {game.group && session && (game.room.status === "playing" || game.room.status === "finished" || game.room.status === "abandoned") && <MultiplayerGame group={game.group} players={game.players} meId={game.meId} busy={busy} act={act} locale={locale} roomCode={session.code} sessionToken={session.token} welcomeAck={game.room.welcomeAck} canRestart={Boolean(host && game.room.canHostMultiplayer)} />}
    {game.room.status === "playing" && !game.group && <GameTable game={game} busy={busy} act={act} />}
    {game.room.status === "finished" && !game.group && !reveal && <Finished players={game.players} restart={() => act("newTable")} />}
    {game.room.status === "abandoned" && !game.group && <Abandoned leave={leave} />}
...
function LegacyLobby({ game, host, busy, copied, copyInvite, act }: { game: GameState; host: boolean; busy: boolean; copied: boolean; copyInvite: () => void; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  return <section className="lobby"><div className="lobby-head"><span className="eyebrow">{ja ? "カードをシャッフル中" : "SHUFFLING THE CARDS"}</span><h1>{ja ? <>デッキの準備は<em>ほぼ</em>完了です。</> : <>Your deck is <em>almost</em> ready.</>}</h1><p>{game.room.multiplayer ? (ja ? "3〜6人で遊べます。招待された人は無料で参加できます。" : "Bring 3–6 players together. Your guests can join for free.") : (ja ? "一緒に遊ぶ人を1人招待してください。このテーブルは2人専用です。" : "Invite one person. This table has exactly two seats.")}</p></div><div className="lobby-grid"><div className="panel"><div className="panel-title"><h2>{ja ? "テーブル" : "At the table"} <small className="room-name-tip" title={`${ja ? "ルーム" : "Room"} ${game.room.code}`}>{ja ? "ルーム" : "ROOM"} {game.room.code}</small></h2><span>{game.players.length}/{game.room.multiplayer ? 6 : 2}</span></div><div className="people-list">{game.players.map((player, index) => <div className="person" key={player.id}><span className={`avatar avatar-${index}`}>{player.name[0]}</span><div><strong>{player.name}</strong><small>{player.isHost ? (ja ? "ホスト" : "host") : (ja ? "参加済み" : "ready to play")}</small></div>{host && game.room.multiplayer && player.id !== game.meId ? <button className="tiny-button" type="button" disabled={busy} aria-label={ja ? `${player.name}を退出させる` : `Remove ${player.name}`} onClick={() => { if (window.confirm(ja ? `${player.name}をテーブルから退出させますか？` : `Remove ${player.name} from the table?`)) void act("removePlayer", { playerId: player.id }); }}>×</button> : <i>●</i>}</div>)}</div><button className="invite-button" onClick={copyInvite}>{copied ? (ja ? "リンクをコピーしました ✓" : "LINK COPIED! ✓") : (ja ? "招待リンクをコピー" : "COPY INVITE LINK")}</button></div><div className="panel"><div className="panel-title"><h2>{ja ? "デッキ" : "The deck"}</h2><span className="sticker">{ja ? "7種類のカード" : "7 CARD TYPES"}</span></div><div className="setting"><label>{ja ? "カード枚数" : "Number of cards"}</label><div className="segmented">{[8, 12, 16, 24].map((count) => <button key={count} disabled={!host} className={roundCount === count ? "active" : ""} onClick={() => { setRoundCount(count); void configure({ roundCount: count }); }}>{count}</button>)}</div></div><div className="setting"><label>{ja ? "テーマカテゴリ" : "Theme categories"}</label><p className="setting-hint">{ja ? "質問カードと心読みカードの内容を決めます。" : "These guide the question and preference cards."}</p><div className="subject-checks">{THEME_KEYS.map((key) => <label className={`subject-check ${key === "spicy" ? "spicy-check" : ""} ${selected.includes(key) ? "selected" : ""} ${key === "spicy" && !canUseSpicy ? "locked" : ""}`} key={key}><input type="checkbox" checked={selected.includes(key)} disabled={!host || (key === "spicy" && !canUseSpicy)} onChange={() => toggleTheme(key)} /><span>{ja ? THEME_LABELS_JA[key] : THEME_LABELS[key]}{key === "spicy" && !canUseSpicy && <small className="premium-lock">PREMIUM</small>}</span></label>)}</div></div><div className="setting"><label>{ja ? "追加テーマ（任意）" : "Optional custom subject"}</label><input className="custom-setting" defaultValue={game.room.customTheme ?? ""} disabled={!host} placeholder={ja ? "例：旅行の思い出" : "e.g. our travel stories"} onBlur={(event) => configure({ customTheme: event.target.value })}/></div>{host ? <button className="primary-button start-button" disabled={!lobbyAccess({ multiplayer: Boolean(game.room.multiplayer), count: game.players.length, isHost: host, premium: Boolean(game.room.canHostMultiplayer), busy }).canStart} onClick={() => act("start")}>{game.players.length >= (game.room.multiplayer ? 3 : 2) ? (game.room.multiplayer && !game.room.canHostMultiplayer ? (ja ? "ホストのPREMIUMが必要です" : "PREMIUM HOST REQUIRED") : (ja ? "シャッフルして開始 →" : "SHUFFLE & START →")) : (game.room.multiplayer ? (ja ? "最低3人の参加を待っています…" : "WAITING FOR AT LEAST 3 PLAYERS…") : (ja ? "2人目の参加を待っています…" : "WAITING FOR PLAYER TWO…"))}</button> : <div className="host-note">{ja ? "ホストがデッキを設定しています。" : "The host is choosing the deck."}</div>}</div></div></section>;
}
...
function Lobby(props: { game: GameState; host: boolean; busy: boolean; copied: boolean; copyInvite: () => void; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  const multiplayer = Boolean(game.room.multiplayer);
  const deckKeys = multiplayer ? multiplayerDeckKeys : [...DECK_KEYS];
  const access = lobbyAccess({ multiplayer, count: game.players.length, isHost: props.host, premium: Boolean(game.room.canHostMultiplayer), busy: props.busy });
  const deckLabel = (key: string) => key === "who" ? (ja ? "この中で誰？" : "WHO AT THE TABLE?") : key === "challenge" ? (ja ? "チャレンジ" : "CHALLENGES") : key === "both" && multiplayer ? (ja ? "みんなで飲む" : "EVERYONE DRINKS") : ja ? CARD_META_JA[key as keyof typeof CARD_META_JA].label : CARD_META[key as keyof typeof CARD_META].label;
  const canCustomizeDeck = Boolean(game.room.canCustomizeDeck); const [selected, setSelected] = useState<string[]>(() => String(game.room.cardTypes ?? deckKeys.join(",")).split(","));
  const [deckOpen, setDeckOpen] = useState(false);
  useEffect(() => { setSelected(String(game.room.cardTypes ?? (game.room.multiplayer ? multiplayerDeckKeys : DECK_KEYS).join(",")).split(",")); }, [game.room.cardTypes, game.room.multiplayer]);
