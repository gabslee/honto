...
function AccountControl() {
  ...
  if (Boolean(user)) return createPortal(<div className="account-stack"><span className="account-control signed-in-account"><span>{user!.displayName ?? user!.email}</span><button type="button" aria-label="Sign out" onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); setUser(null); }}>×</button></span>{user!.role === "admin" ? <span className="account-control signed-in-account">ADMIN · UNLIMITED</span> : user!.plan === "premium" || user!.subscriptionStatus === "active" ? user!.stripeCustomerId ? <button type="button" className="account-control curated-button" onClick={() => void openPortal()}>MANAGE PREMIUM</button> : <span className="account-control signed-in-account">PREMIUM ACTIVE</span> : <button type="button" className="account-control curated-button" onClick={() => { window.location.href = "/pricing"; }}>START 7-DAY PREMIUM TRIAL</button>}</div>, mount);
  const content = !user ? <a className="account-control curated-button" href="/api/auth/google/start"><svg className="google-mark" viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M21.35 12.27c0-.72-.06-1.42-.18-2.09H12v3.96h5.24a4.48 4.48 0 0 1-1.94 2.94v2.45h3.14c1.84-1.7 2.91-4.2 2.91-7.26Z"/><path fill="#34A853" d="M12 21.7c2.63 0 4.84-.87 6.45-2.37l-3.14-2.45c-.87.58-1.98.92-3.31.92-2.54 0-4.7-1.72-5.47-4.03H3.29v2.53A9.74 9.74 0 0 0 12 21.7Z"/><path fill="#FBBC05" d="M6.53 13.77A5.85 5.85 0 0 1 6.22 12c0-.62.11-1.22.31-1.77V7.7H3.29A9.74 9.74 0 0 0 2.25 12c0 1.57.38 3.05 1.04 4.3l3.24-2.53Z"/><path fill="#EA4335" d="M12 6.2c1.43 0 2.72.49 3.73 1.45l2.8-2.8C16.84 3.25 14.63 2.3 12 2.3a9.74 9.74 0 0 0-8.71 5.4l3.24 2.53C7.3 7.92 9.46 6.2 12 6.2Z"/></svg>SIGN IN WITH GOOGLE</a> : <span className="account-control signed-in-account">{user.displayName ?? user.email}</span>;
...
function Lobby(props: { game: GameState; host: boolean; busy: boolean; copied: boolean; copyInvite: () => void; act: (action: string, extras?: Record<string, unknown>) => Promise<unknown> }) {
  ...
  return <div className="lobby-composite"><LegacyLobby {...props}/><section className="lobby deck-filter-lobby"><div className="panel deck-filter-panel"><div className="panel-title"><h2>{ja ? "カードの種類" : "Card types"}</h2><span className="sticker">{canCustomizeDeck ? (ja ? "PREMIUM" : "PREMIUM") : (ja ? "ロック中" : "LOCKED")}</span></div><p className="setting-hint">{canCustomizeDeck ? (ja ? "使いたいカードだけを選べます。順番はシャッフルされます。" : "Choose the games you want. The order stays shuffled.") : (ja ? "自分のデッキを選ぶ機能はPremiumです。" : "Custom deck selection is a Premium feature.")}</p><div className="subject-checks card-type-checks">{DECK_KEYS.map((key) => <label className={`subject-check card-type-check ${selected.includes(key) ? "selected" : ""} ${!canCustomizeDeck ? "locked" : ""}`} key={key}><input type="checkbox" checked={selected.includes(key)} disabled={!props.host || !canCustomizeDeck || (selected.length <= 3 && selected.includes(key))} onChange={() => toggle(key)}/><span>{ja ? CARD_META_JA[key].label : CARD_META[key].label}{!canCustomizeDeck && <small className="premium-lock">PREMIUM</small>}</span></label>)}</div><small className="deck-filter-note">{ja ? "最低3種類。枚数と順番は自動で決まります。" : "Choose at least 3 types. Counts and order stay automatic."}</small></div></section></div>;
}
