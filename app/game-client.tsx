...
function AccountControl() {
  const [user, setUser] = useState<{ email: string; displayName?: string; plan?: string; subscriptionStatus?: string; trialEndsAt?: string | null; stripeCustomerId?: string | null } | null>(null);
  ...
  if (Boolean(user)) return createPortal(<div className="account-stack"><span className="account-control signed-in-account"><span>{user!.displayName ?? user!.email}</span><button type="button" aria-label="Sign out" onClick={async () => { await fetch("/api/auth/logout", { method: "POST" }); setUser(null); }}>×</button></span>{user!.plan === "premium" || user!.subscriptionStatus === "active" ? <button type="button" className="account-control curated-button" onClick={() => void openPortal()}>MANAGE PREMIUM</button> : <button type="button" className="account-control curated-button" onClick={() => void openCheckout("month")}>START 7-DAY PREMIUM TRIAL</button>}</div>, mount);
