...
test('draw belongs to the author; anonymous players are not labeled disconnected', () => {
  const s = state();
  assert.match(html(s), />DRAW</);
  assert.doesNotMatch(html(s, 'b'), />DRAW</);
...
test('secret wagers and welcome acknowledgment gate the first card', () => {
  ...
  assert.match(output, /Your secret challenge/i);
...
test('multiplayer reuses the original Honto card, draw, AI and wheel design', () => {
  const preparing = action(state(), 'a', 'draw');
  const preparation = html(preparing);
  assert.match(preparation, /class="play-card writer"/);
  assert.match(preparation, /GENERATE 5 LIES/);
  assert.match(source, /honto-playing-card/);
  assert.match(source, /sip-wheel sip-wheel-stop-/);
  assert.doesNotMatch(source, /mp-wheel-rotor|multiplayer\.css/);
});
